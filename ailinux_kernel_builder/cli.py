...
def install_dependencies() -> int:
    ...
    cmd: list[str] = []
    if os.geteuid() == 0:
        cmd = ["/usr/bin/apt-get", "install", "-y", *missing]
    elif shutil.which("pkexec") and "DISPLAY" in os.environ:
        cmd = ["pkexec", "/usr/bin/apt-get", "install", "-y", *missing]
    elif shutil.which("sudo"):
        cmd = ["sudo", "/usr/bin/apt-get", "install", "-y", *missing]
    else:
        cmd = ["/usr/bin/apt-get", "install", "-y", *missing]
...
def build_kernel_cli(archive_path: Path, options: BuildOptions, app_dir: Path) -> int:
    ...
    try:
        ...
        if options.install_after_build:
            ...
            install_cmd = ["sudo", "/usr/bin/apt-get", "install", "-y", *[str(p) for p in install_pkgs]]
